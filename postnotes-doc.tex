...
``if the marks of my notes are in numeric order'', but rather whether the
order the marks are typeset along the document and the order the notes are
printed in \cs{printposnotes} are the same.  The \opt{check} option is a
convenience alias to set both checks at the same time.  These options are
booleans and are initially disabled.
