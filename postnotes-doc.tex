...
typeset.  These problems are usually not too frequent, but they can be a pain.
Hence the \opt{checkduplicates} and \opt{checkfloats} options, which issue
warnings for these cases.  The basis for these checks are the page labels each
\cs{postnote} sets internally.  These labels are non-immediate writes
(whatsits, in the jargon), as they must be, since we are interest in the page
number.  As a result, measuring passes don't get written, and they are also
written in the order the document elements get shipped out.  Hence, the labels
set in the \file{.aux} file are free of duplicates resulting from measuring
passes, and are ordered the same as the typeset order of the document, floats
considered.  Note that these checks are only expected to be meaningful in a
document for which the compilation has stabilized: with labels and floats
already in place.  Also note that the \opt{checkfloats} option does not test
``if my notes are in numeric order'' (that's the job of \opt{sort}), but
rather whether the order the marks are typeset along the document and the
order the notes are printed in \cs{printposnotes} are the same.  These options
are booleans.  \opt{checkduplicates} is initially enabled, since whether a
place performs measuring passes does not really change, and it is easy to deal
with a note in this situation with the \opt{maybemulti}
option. \opt{checkfloats}, on the other hand, is initially disabled.  Float
placement (and its implications for \pkg{postnotes}) is something one should
not worry about until the very last stages of the document.  At that time, I
expect it to be useful, but during the writing phase, such a warning would
only be distracting.  Think of if as something to use like you would use the
\pkg{widows-and-orphans} package, for example.
...
(internal) page label exists.  It's the same principle used for
\opt{checkduplicates}, but instead of getting the warning, here you flag the
duplicate for potential dropping.  The drawback is that, since it requires the
label for the check, a new note set with \opt{maybemulti} will need one more
compilation run than otherwise.  This is a boolean option, and the initial
value is false.
...
notes as well\footnote{The second one could be made do work with multiple
  notes too with a suitable arrangement of \cs{refstepcounter}, \cs{label},
  and \cs{getrefnumber}.}, and it is also more robust to multiple passes of
the content.  \pkg{postnotes} controls for this in a number of known cases to
avoid notes in duplicity, but obviously cannot cover every possibility, in
which case \opt{nomark} with \cs{postnoteref} has you covered.  Caveat, these
techniques assume the floats do not disturb the order of the notes, otherwise
we are back to the case discussed in \zcref{ex:x:float}.
