...
\opt{heading} (with \cs{markboth} or \cs{markright}).  So, the default headers
are enabled through that particular setting.
...
\NewDocumentCommand \mypnheader {}
  {
    \bool_case_true:nF
      ...
      }
      {
        \tl_if_eq:NNTF \pnhdchapfirst \pnhdchaplast
          { Notes~to~chapter~\pnhdchapfirst{},~ }
          { Notes~to~chapter~\pnhdchapfirst{} -- \pnhdchaplast{},~ }
      }
    \tl_if_eq:NNTF \pnhdpagefirst \pnhdpagelast
      { page~\pnhdpagefirst{} }
      { pages~\pnhdpagefirst{} -- \pnhdpagelast{} }
